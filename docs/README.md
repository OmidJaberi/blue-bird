# Blue-Bird Documentation

Welcome to the official documentation for Blue-Bird.

Blue-Bird is a modular backend and web framework written in C, focused on:
- explicit architecture
- low overhead
- composable infrastructure
- backend-oriented development

---

# Guides

## Core Guides

- [Getting Started](getting-started.md)
- [Architecture](architecture.md)
- [Philosophy](philosophy.md)
- [Naming Conventions](naming_conventions.md)

---

# Web Module

- [Web Overview](web/overview.md)
- [Routing](web/routing.md)
- [Middleware](web/middleware.md)

---

# Persistence Module

- [Persistence Overview](persist/overview.md)
- [Models](persist/models.md)
- [Schemas](persist/schemas.md)

---

# Template Module

- [Template Overview](template/overview.md)

---

# Runtime Module

- [Runtime Overview](runtime/overview.md)

---

# Security Module

- [Security Overview](security/overview.md)
- [Auth](security/auth.md)
- [Passwords](security/passwords.md)
- [Sessions](security/sessions.md)
- [Usage](security/usage.md)

---

# Log Module

- [Log Overview](log/overview.md)
- [API](log/api.md)
- [Back Ends](log/backends.md)

---

# Error Module

- [Error Overview](error/overview.md)

---

# Utilities

- [JSON](utils/json.md)
- [UUID](utils/uuid.md)